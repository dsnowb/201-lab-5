...
function sumArray(testArray){ //eslint-disable-line
  ...
  for (var i = 0; i < testArray.length; i++) {
    theSum = sum(theSum,testArray[i]);
    theSum = theSum[0];
    theSumMessage += testArray[i];
    if (i !== testArray.length - 1) theSumMessage += ',';
  }

  theSumMessage += ' was passed in as an array of numbers, and ' + theSum + ' is their sum.';
  return [theSum,theSumMessage];
...
function multiplyArray(testArray){ //eslint-disable-line
  var theProduct = 1;
  var theProductMessage = '';
  for (var i = 0; i < testArray.length; i++) {
    theProduct = multiply(theProduct,testArray[i]);
    theProduct = theProduct[0];
    theProductMessage += testArray[i];
    if (i !== testArray.length -1) theProductMessage += ',';
  }

  theProductMessage = 'The numbers ' + theProductMessage + ' have a product of ' + theProduct + '.';
  return [theProduct,theProductMessage];
}
...
function writeToEl(someEl,someText) {
...
writeToEl('sum',sum(4,7)[0]);
writeToEl('multiply',multiply(5,9)[0]);
writeToEl('sumOfSAM',sumAndMultiply(4,7,5)[0]);
writeToEl('prodOfSAM',sumAndMultiply(4,7,5)[1]);
writeToEl('sumArray',sumArray(testArray)[0]);
writeToEl('multiplyArray',multiplyArray(testArray)[0]);
